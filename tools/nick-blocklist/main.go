...
// atMost drops fragments longer than the longest possible nick: they can
// never match and would only make the file look more thorough than it is.
func atMost(list []string, maxLen int) []string {
	kept := list[:0:0]
...
func run(args []string, stdout, stderr io.Writer) error {
	...
	maxLen := fs.Int("max", 9, "longest fragment kept; a longer one can never match a nick")
	attribution := fs.String("attribution", "", "source and licence credit required by the input lists, e.g. CC-BY-4.0")
	...
	list := atMost(build(entries, *minLen), *maxLen)
	...
	b.WriteString("# Alphabet: " + alphabet + "; fragment length: " + fmt.Sprint(*minLen) + "–" + fmt.Sprint(*maxLen) + "\n")
	b.WriteString("# Sources: " + strings.Join(sources, ", ") + "\n")
	if line := strings.Join(strings.Fields(*attribution), " "); line != "" {
		b.WriteString("# Attribution: " + line + "\n")
	}
	for _, w := range list {
		b.WriteString(w + "\n")
	}
