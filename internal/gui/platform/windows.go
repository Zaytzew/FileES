...
func buildReservationDialogScript(request ReservationDialogRequest) (string, error) {
	...
	sb.WriteString("$t=New-Object System.Data.DataTable;[void]$t.Columns.Add('ID');[void]$t.Columns.Add('Serwer');[void]$t.Columns.Add('Katalog roboczy');[void]$t.Columns.Add('Plik');[void]$t.Columns.Add('Właściciel');[void]$t.Columns.Add('Utworzono');[void]$t.Columns.Add('Zwolnienie');foreach($r in $d.Rows){[void]$t.Rows.Add($r.ID,$r.Server,$r.WorkingCopy,$r.Path,$r.Owner,$r.CreatedAt,$r.ReleaseStatus)};$g.DataSource=$t;$g.Columns['ID'].Visible=$false;$f.Controls.Add($g);")
