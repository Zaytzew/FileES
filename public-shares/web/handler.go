...
func freezeListingTree(node *listingTreeBuilder, bundles bool) listingDirectory {
	result := listingDirectory{Name: node.name, Path: node.path, Files: append([]listingFile(nil), node.files...), BundleEnabled: bundles, FolderIcon: brandSymbol, DownloadIcon: listingIcon("download")}
...
func formatListingTotalSize(objects []channel.PublicObject) string {
	var total int64
	known := 0
	for _, object := range objects {
		if object.Size == nil {
	...
	if len(objects) > 0 && known == 0 {
		return "rozmiar nieznany"
	}
	formatted := formatListingSize(total)
	if known < len(objects) {
		return "co najmniej " + formatted
	}
	return formatted
}
...
var brandWordmark = func() template.HTML {
	raw, err := embeddedWebAssets.ReadFile("assets/brand/filees-space-horizontal.svg")
	if err != nil || !strings.HasPrefix(strings.TrimSpace(string(raw)), "<svg") {
		panic("invalid embedded filees:space wordmark")
	}
	return template.HTML(raw) // Owner-supplied, pinned SVG asset.
}()
...
<body>
<main class="shell">
<header class="topbar"><div class="brand"><span class="brand-mark" aria-hidden="true">{{.BrandSymbol}}</span><span class="brand-wordmark" aria-label="filees:space">{{.BrandWordmark}}</span></div><div class="realm">udostępnione przez {{.Alias}}</div></header>
<section class="card" aria-labelledby="share-title">
<div class="heading"><div><h1 id="share-title">{{.Slug}}</h1><div class="meta"><span>{{.TotalSize}}</span><span>opublikowano {{.PublishedDate}}</span><span><a class="latest-link" href="{{.LatestURL}}">Sprawdź najnowsze wydanie</a></span></div></div>{{if .HasOwnerLogo}}<img class="owner-logo" src="{{.OwnerLogo}}" alt="Logo udostępniającego">{{end}}</div>
<div class="browser">
{{if eq .Count 0}}
