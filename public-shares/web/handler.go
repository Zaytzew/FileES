...
func (h Handler) renderListingWithInvitation(w http.ResponseWriter, projection channel.Projection, claims visit, encoded, invitation string, selectionNotice bool) {
	...
	data := listingPage{
		...
		StateDate:       formatListingDate(projection.UpdatedAt),
...
<div class="heading"><div><h1 id="share-title">{{.Slug}}</h1><div class="meta"><span>{{.TotalSize}}</span><span>stan {{.StateDate}}</span><span><a class="latest-link" href="{{.LatestURL}}">Sprawdź najnowsze wydanie</a></span></div></div>{{if .HasOwnerLogo}}<img class="owner-logo" src="{{.OwnerLogo}}" alt="Logo udostępniającego">{{end}}</div>
<div class="browser">
{{if eq .Count 0}}
