...
// unportableRenameClient is the one daemon call the rename needs.
type unportableRenameClient interface {
	RepoRenameUnportable(ctx context.Context, payload contract.RepoRenameUnportablePayload) error
}

type unportableRenameAdapter struct{ client unportableRenameClient }

func (adapter unportableRenameAdapter) RenameUnportable(ctx context.Context, serverID, repoID, path, newName string) error {
	if adapter.client == nil {
		return errors.New("rename client is unavailable")
	}
	return adapter.client.RepoRenameUnportable(ctx, contract.RepoRenameUnportablePayload{ServerID: serverID, RepoID: repoID, Path: path, NewName: newName})
}
