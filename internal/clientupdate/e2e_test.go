...
func TestSignedSVNReleaseEndToEnd(t *testing.T) {
	...
	for _, installed := range []string{".local/bin/filees", ".local/bin/filees-gui", ".local/bin/filees-pair-gui", ".local/bin/filees-svn", ".config/filees/config.json"} {
...
func makeE2EBundle(t *testing.T, root string) string {
	...
	for _, name := range []string{"VERSION", "bin/filees", "bin/filees-gui", "bin/filees-pair-gui", "bin/filees-svn", "share/applications/filees-gui.desktop", "share/filees/config.example.json", "share/icons/hicolor/scalable/apps/filees-gui.svg", "share/systemd/user/filees.service"} {
