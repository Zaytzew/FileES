...
func buildSettingsDialogScript(request SettingsDialogRequest) (string, error) {
	...
	for _, spec := range []struct {
		label, action string
		left          int
	}{{"Widoczność", "realm_visibility", 75}, {"Dostęp stref", "manage_grants", 190}, {"Dodaj folder", "add", 305}, {"Odłącz folder", "detach", 420}, {"Odłącz trwale", "delete", 535}, {"Odtwórz z archiwum", "load_dump", 650}, {"Dezaktywuj klienta", "deactivate", 765}, {"Usuń udział FileES", "remove_realm", 880}, {"Pobierz archiwa", "download_recovery", 995}} {
		variable := "$b"
